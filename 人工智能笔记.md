...
+ 它能学习样本数据的内在规律和表示层次，能自动从图像中学习有效的特征。
  + 深度学习来源于人工神经网络，之所以叫“深度”是因为它自身由许多不同的层组成。
    ...
    + 经过很多层的变换之后，这种结构就可以组合原始图像中的低层特征形成更加抽象的高层特征。这种由简单到复杂、由低级到高级的抽象过程，可以通过生活中的例子来体会。
### 那深度学习和机器学习有何关系呢？
...
<img width="1754" height="969" alt="屏幕截图_20-8-2026_9527_np baicizhan com" src="https://github.com/user-attachments/assets/949a6303-ddef-417c-8987-832a74b1a87f" />
...
1. 卷积神经网络 — CNN（受到人类视觉神经系统的启发，最擅长进行图像处理的一种算法。）, 循环神经网络 — RNN,
2. 生成对抗网络 — GANs, 深度强化学习 — RL（游戏）。

## 卷积神经网络
+ 卷积神经网络是深度学习的代表算法之一。
  + 其本质是一个多层结构，这些层可以划分三个部分，输入层、隐含层和输出层组成。
  + 一般连接顺序为输入层-隐含层-输出层。
  + 在图像识别中，输入层输入的是图像，输出层输出的是识别结果
  + 那什么是隐含层呢？

     + 隐含层主要由卷积层、池化层和全连接层3类常见的结构组成。
     + 连接顺序通常为：卷积层（负责提取图像中的局部特征。）-池化层（负责⼤幅降低参数量级(简称降维)）-全连接层（起到“分类器”的作用，负责对特征进行重新计算，减少信息的丢失。）
       <img width="1474" height="981" alt="屏幕截图_20-8-2026_1010_np baicizhan com" src="https://github.com/user-attachments/assets/d11c57b9-5c9a-41ac-ba5d-eefe23c1fbf3" />
       
### 卷积神经网络的应用
卷积神经网络在图像领域的应用处处可见，比如：
1. 图像分类、检索
2. 目标定位检测，在图像中定位目标，并确定目标的位置及大小。
3. 目标分割，简单理解就是一个像素级的分类。
4. 人脸识别
5. 骨骼识别
